 ...
 * @return {Object}
 */
export async function dataTable2Object(ctx: QavajsPlaywrightWdioWorld, dataTable: DataTable): Promise<{ [key: string]: string }> {
 ...
 * @return {any[]}
 */
export function dataTable2Array(ctx: QavajsPlaywrightWdioWorld, dataTable: DataTable): Promise<any[]> {
    return Promise.all(dataTable.raw().map(([value]) => ctx.value(value)));
}

export async function sendHttpRequest(world: QavajsPlaywrightWdioWorld, requestUrl: string, options: Record<string, string>): Promise<APIResponse> {
