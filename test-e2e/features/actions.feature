Feature: actions
  ...
  Scenario: resize browser's window
    ...
    Then I execute '$js(() => ({ width: window.outerWidth, height: window.outerHeight }))' script and save result as 'viewport'
    Then I expect '$viewport' to deeply equal '$js({ width: 640, height: 480 })'
    And I set window size '800,600'
    Then I execute '$js(() => ({ width: window.outerWidth, height: window.outerHeight }))' script and save result as 'viewport'
    Then I expect '$viewport' to deeply equal '$js({ width: 800, height: 600 })'
